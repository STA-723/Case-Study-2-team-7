...
set.seed(1234)
subset<- sample(1:nrow(AB_NYC_2019_complete),10000)
...
hash.df <- hashed.model.matrix(~ split(name, delim = " ", type = "tf-idf"),
                          data = AB_subset, hash.size = 2^9, signed.hash = FALSE)
...
keepterm <- findFreqTerms(tdm, lowfreq = 20, highfreq = Inf)
tdm2 <- tdm[,keepterm]
...
age.bas = bas.lm(price~ ., prior = 'JZS', modelprior=uniform(), method="MCMC", pivot=TRUE,  # for non-full rank case
                   n.models=50000,
                   force.heredity = TRUE, data = AB_subset)
summary(age.bas)
```